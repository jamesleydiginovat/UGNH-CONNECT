...
<div x-show="!tableSlide" @class([
    ...
    <div @class([
        ...
        <button
            @click="{{ !$isBlocked ? 'form = !form' : 'null' }}"
            @class([
                'flex flex-row p-2 rounded gap-2 transition-all duration-300 justify-between items-center',

                // ACTIF (autorisé)
                'bg-ugnh-blueFonce text-gray-50 hover:bg-ugnh-blueHover hover:shadow-lg' => !$isBlocked,

                // BLOQUÉ (secrétaire + vice doyen)
                'opacity-70 cursor-not-allowed bg-gray-300 text-gray-600' => $isBlocked,
        ...
        <!-- Info box -->
        <p class="p-2 bg-ugnh-blueClair dark:bg-gray-800 dark:text-gray-300 rounded">
            Acceder aus differentes fonctionnalite!
        </p>

    </div>
</div>

<div x-show="!tableSlide"
     x-cloak
    ...
    <livewire:pages.cours.tableau-cours />
</div>
...
<div 
    ...
    <livewire:pages.cours.tableau-horaires-fac />
</div>

    <livewire:pages.cours.formulaire-create-cours />
