            ...
            <div  @class([
                ...
                'dark:border-gray-600'
                  ...
                  <div class="flex flex-col gap-5 border border-gray-200 shadow-sm p-6 dark:border-gray-600">
                            ...
                            @error('conflit')
                                <div class="flex items-center gap-2 bg-red-50 border border-red-300 text-red-600 px-3 py-2 rounded">
                                    ⚠️ {{ $message }}
                                </div>
                            @enderror
                            ...
                            <div class=" flex flex-row">
                                <label class="font-bold text-gray-500"  for="cours">
                                    Cours
                                </label>
                                 <div class="relative w-[90%]  pb-4">
                                <select wire:model.live="cours" @class([
                                'outline-0 border-b border-gray-600 mx-3 w-full',
                                'dark:text-ugnh-blueClair dark:border-gray-600'
                                ,'border-red-600'=>$errors->has('cours')]) name="" id="cours">
                                    {{-- <option value=""></option> --}}
                                    {{-- <option class="dark:text-ugnh-blueFonce" value="">Choisir le personnel</option> --}}
                                    
                                    <option class="dark:text-gray-200 dark:bg-gray-700" value="">Choisir le cours</option>
                                    @if ($codeFac != "" && $niveau !="" && $session !="")
                                        @if (!empty($this->CoursByFaculte))
                                            @foreach ($this->CoursByFaculte as $cours)
                                            <option class="dark:text-gray-200 dark:bg-gray-700" value="{{  $cours->codeCours  }}">{{ $cours->nom }}</option>
                                             @endforeach 
                                        @elseif(empty($this->CoursByFaculte))
                                            <option class="dark:text-gray-200 dark:bg-gray-700" value="">Aucun cours disponible</option>
                                        @endif
                                        
                                    @else
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="">Selectionner quel faculte, quel niveau et quel session</option>
                                    @endif
                                
                                    
                                    {{-- <option class="dark:text-ugnh-blueFonce" value="2">Comptabilite</option> --}}
                                </select>
                                @error('cours') <p class="text-red-500 text-[10px] absolute  mx-3 bottom-0">{{ $message }}</p> @enderror
                                </div>
                            </div>
                             ...
                             <div class=" flex flex-row">
                                <label class="font-bold text-gray-500"  for="jour">
                                    Jours
                                </label>
                                <div class="relative w-[90%]  pb-4">
                                <select wire:model.live="jour" @class([
                                'outline-0 border-b border-gray-600 mx-3 w-full',
                                'dark:text-ugnh-blueClair dark:border-gray-600'
                                ,'border-red-600'=>$errors->has('jour')]) name="" id="jour">
                                    {{-- <option value=""></option> --}}
                                    {{-- <option class="dark:text-ugnh-blueFonce" value="">Choisir le personnel</option> --}}
                                    <option class="dark:text-gray-200 dark:bg-gray-700" value="">Choisir le jour</option>
                                    {{-- @foreach ($this->Professeurs as $professeur) --}}
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Lundi">Lundi</option>
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Mardi">Mardi</option>
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Mercredi">Mercredi</option>
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Jeudi">Jeudi</option>
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Vendredi">Vendredi</option>
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Samedi">Samedi</option>
                                         <option class="dark:text-gray-200 dark:bg-gray-700" value="Dimanche">Dimanche</option>
                                         {{-- <option class="dark:text-ugnh-blueFonce" value="Lundi">Lundi</option> --}}
                                    {{-- @endforeach --}}
                                </select>
                                @error('jour') <p class="text-red-500 text-[10px] absolute  mx-3 bottom-0">{{ $message }}</p> @enderror
                                </div>
                            </div>
                            ...
                            <div class=" flex flex-row">
                                <label class="font-bold text-gray-500"  for="heure_debut">
                                    Heure debut
                                </label>
                                <div class="relative w-[80%] pb-4">
                                <input @readonly(false) wire:model.live="heure_debut" @class(['outline-0 border-b border-gray-600 w-full mx-3','border-red-600'=>$errors->has('heure_debut')]) type="time" id="codeCours" >
                                @error('heure_debut') <p class="text-red-500 text-[10px] absolute  mx-3 bottom-0">{{ $message }}</p> @enderror
                                </div>
                            </div>
                            ...
                            <div class=" flex flex-row">
                                ...
                                <div class="relative w-[80%] pb-4">
                                <input @readonly(false) wire:model.live="heure_fin" @class(['outline-0 border-b border-gray-600 w-full mx-3','border-red-600'=>$errors->has('heure_fin')]) type="time" id="heure_fin" >
                                @error('heure_fin') <p class="text-red-500 text-[10px] absolute  mx-3 bottom-0">{{ $message }}</p> @enderror
                                </div>
                            </div>
