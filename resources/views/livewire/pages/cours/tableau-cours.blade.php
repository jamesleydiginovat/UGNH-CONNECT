<div class="mx-3 text-gray-700  dark:text-gray-300 bg-white dark:bg-gray-900 p-3 rounded-t-lg  ">
    ...
    <div @class([
		...
		@class([
            ...
            <div :class="!inpuRecherche ? '' : 'md:hidden'" @class([
				'flex flex-col sm:flex-row flex-wrap items-stretch gap-2 w-full',
				'md:w-auto'
			])>
				...
				<div @class([
					...
					<select wire:model.live="byNiveau" @class([
						...
						<option class="dark:text-gray-200 dark:bg-gray-700" value="">Niveau</option>
				...
				<div @class([
					...
					<select wire:model.live="bySession" @class([
						...
						<option class="dark:text-gray-200 dark:bg-gray-700" value="">Session</option>
						<option class="dark:text-gray-200 dark:bg-gray-700" value="1">I</option>
						<option class="dark:text-gray-200 dark:bg-gray-700" value="2">II</option>
					</select>
				</div>
		...
		<table class="min-w-full text-xs ">
			...
			<tbody>
				@foreach ($this->LesCours as $cours)
					<tr class="border-b border-[#ccc] dark:hover:bg-gray-800 dark:border-gray-600 hover:bg-ugnh-blueClair transition-all duration-200 ease-in-out ">
						...
						<td class="p-1 flex flex-row justify-center">
							...
							@endphp
							@if (!$isBlocked)
								<svg wire:click="sessionEdit({{ $cours->id }})" @click="form = !form, $dispatch('open-modal')" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 cursor-pointer  hover:scale-120">
							...
							@php
								...
								// ❌ rôles bloqués
								$isBlocked = in_array($role, ['Secrétaire faculté','Vice-doyen de faculté']);
							@endphp
							@if (!$isBlocked)
							<svg wire:click="selectionCours({{ $cours->id }})"  @click="modalConfirmation = !modalConfirmation" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-red-500 cursor-pointer hover:text-red-600 hover:scale-120">
    ...
    <div class="absolute z-10  w-full m-auto h-full flex flex-row  items-center justify-center">
        ...
        <div class="bg-white relative rounded-lg p-5 dark:bg-gray-800 lg:min-w-[40%]  sm:min-w-[50%] sm:w-auto w-[93%]  h-auto  shadow-2xl overflow-y-auto ">
            ...
            <div class="flex sm:flex-row  flex-col">
                ...
                <div class="">
                    <h1 class="font-bold ">Suppression d'un cours</h1>
                    ...
                    <div class=" mt-5 flex fle-col gap-3 sm:justify-end justify-between">
                        <button wire:click="deleteCours({{ $CoursSelectionner }})"  @click="modalConfirmation = !modalConfirmation" class="bg-red-600 p-2 w-25 rounded-lg text-gray-50 hover:bg-red-400">Supprimer</button>
				...
				<div class="px-4 py-2 -mx-3">
					<div class="mx-3">
						...
						<p class="text-sm text-yellow-500">
							...
							Aucun cours trouvé.
						@else
							Aucun cours enregistré. Veuillez ajouter un cours!
						@endif</p>
					</div>
				</div>
			</div>
	...
	@if ($this->LesCours->isNotEmpty())
        <div class="mt-4 bottom-0 w-full left-0">
            {{ $this->LesCours->links('pagination::tailwind') }}
        </div>
    @endif
</div>